...
contract LenderTest is Test {
    ...
    function test_startAuction() public {
        ...
        assertEq(startTime, block.number);
    }
    ...
    function test_buyLoan() public {
        test_borrow();
        // accrue interest
        vm.roll(block.number + 1); // @audit-info one day one blockm for testing
        // kick off auction
        vm.startPrank(lender1);
        ...
        // roll to middle of auction
        // @audit-info because we consider one days equal to one block
        vm.roll(block.number + 1);

        lender.buyLoan(0, poolId);

        // assert that we paid the interest and new loan is in our name
        //// @audit-info this part is confusation whey we need gerloandebt after buy loan
        console.log(110 * 10 ** 18, "lender.getLoanDebt(0)", lender.getLoanDebt(0));
    ...
    function test_giveLoanUnevenArrayExecption() public {
        test_borrow();
        ...
        vm.startPrank(lender1);
        vm.expectRevert(ArrayShouldbeSomeSize.selector);
        lender.giveLoan(loanIds, poolIds);
    }
