...
contract Lender is Ownable {
    ...
    function borrow(Borrow[] calldata borrows) public {
        for (uint256 i = 0; i < borrows.length; i++) {
            ...
            Loan memory loan = Loan({
                ...
                startBlockNumber: block.number, // @audit-info don't used time stampe
                auctionStartBlock: type(uint256).max,
            ...
            // transfer fees
            // @audit-info used safe transfer either used  check debt amount not equal to zero
            ...
            loans.push(loan);
            emit Borrowed(msg.sender, pool.lender, loans.length - 1, debt, collateral, pool.interestRate, block.number);
        }
    }
    ...
    // //@audit-info Denial of Service (DoS) Attack: The code is vulnerable to a DoS attack because in the  repay  function, it deletes loans after all operations. If the  IERC20.transfer  function fails, it will stop the execution and the loan will not be deleted.
    function repay(uint256[] calldata loanIds) public {
        for (uint256 i = 0; i < loanIds.length; i++) {
            uint256 loanId = loanIds[i];
            // get the loan info
            Loan memory loan = loans[loanId];
            //maybe attack with DOS
            ////  @audit-info try this one by my self with testing tools
            delete loans[loanId];
            ...
            emit Repaid(
                msg.sender, loan.lender, loanId, loan.debt, loan.collateral, loan.interestRate, loan.startBlockNumber
            );
            // delete the loan
            // delete loans[loanId];
        }
    ...
    function giveLoan(uint256[] calldata loanIds, bytes32[] calldata poolIds) external {
        // @audit-issue if length is not equal of poodid and loanids ✅
        console.log("poolIds.length", poolIds.length);
        if (loanIds.length != poolIds.length) {
            revert ArrayShouldbeSomeSize();
        }
        for (uint256 i = 0; i < loanIds.length; i++) {
            uint256 loanId = loanIds[i];
            ...
            emit Repaid(
                ...
                loan.collateral,
                loan.interestRate,
                loan.startBlockNumber
            );

            // update the loan with the new info
            loans[loanId].lender = pool.lender;
            loans[loanId].interestRate = pool.interestRate;
            loans[loanId].startBlockNumber = block.number; ////@audit-issue don't used time stamp ✅
            loans[loanId].auctionStartBlock = type(uint256).max;
    ...
    function startAuction(uint256[] calldata loanIds) public {
        for (uint256 i = 0; i < loanIds.length; i++) {
            ...
            // validate the loan
            if (msg.sender != loan.lender) revert Unauthorized();
            if (loan.auctionStartBlock != type(uint256).max) {
                revert AuctionStarted();
            }

            // set the auction start timestamp
            //// @audit-issue don't used time stamp used block number ✅
            loans[loanId].auctionStartBlock = block.number;
            emit AuctionStart(
                loan.borrower, loan.lender, loanId, loan.debt, loan.collateral, block.number, loan.auctionLength
            );
        }
    }
    ...
    function buyLoan(uint256 loanId, bytes32 poolId) public {
        ...
        if (block.number > loan.auctionStartBlock + loan.auctionLength) {
            revert AuctionEnded();
        }
        // calculate the current interest rate
        ////   @audit-issue don't used time stamp used block number

        uint256 blockElapsed = block.number - loan.auctionStartBlock;
        // console.log("blockElapsed", blockElapsed);
        uint256 currentAuctionRate = (MAX_INTEREST_RATE * blockElapsed) / loan.auctionLength;
        // validate the rate
        ...
        loans[loanId].startBlockNumber = block.number;
        loans[loanId].auctionStartBlock = type(uint256).max;
        loans[loanId].debt = totalDebt;
    ...
    function seizeLoan(uint256[] calldata loanIds) public {
        for (uint256 i = 0; i < loanIds.length; i++) {
            ...
            if (block.number < loan.auctionStartBlock + loan.auctionLength) revert AuctionNotEnded();
            // calculate the fee
    ...
    function refinance(Refinance[] calldata refinances) public {
        for (uint256 i = 0; i < refinances.length; i++) {
            ...
            require(collateral != 0, "it will be greater then zero");
            // get the loan info
            Loan memory loan = loans[loanId];
            // validate the loan
            ...
            emit Repaid(msg.sender, loan.lender, loanId, debt, collateral, loan.interestRate, loan.startBlockNumber);
            ...
            emit Borrowed(msg.sender, pool.lender, loanId, debt, collateral, pool.interestRate, block.number);
    ...
    function _calculateInterest(Loan memory l) internal view returns (uint256 interest, uint256 fees) {
        // @audit-info block.number will create issues
        // uint256 blockElapsed = block.number - l.startBlockNumber;
        // @audit-issue better to used safe math
        uint256 blockElapsed = block.number - l.startBlockNumber;
        interest = ((l.interestRate * l.debt * blockElapsed) / 10000) / 365;
        // //@audit-info we conceder one block = one day that why 365 you have to fixed on number of block generation for you delpoument project based
